//cssdeck.com/labs/imageslider-pager
//1. set ul width 
...
function initImageSliderPager(){
	ul = document.getElementById('image_slider');
	liItems = ul.children;
	imageNumber = liItems.length;
	imageWidth = 760;//liItems[0].children[0].clientWidth;
...
function slideTo(imageToGo){
	...
	var opts = {
		...
		step:function(delta){
			ul.style.left = parseInt(currentPostion + direction * delta * imageWidth * numOfImageToGo, 10) + 'px';
		},
...
function resizeDiv() {
	console.log('resizeDiv ' + window.innerWidth);
    var zoom = Math.min((window.innerWidth * 0.4) / 800, 1);
	console.log('zoom = ' + zoom);
    
    $('.container').css({ '-webkit-transform': 'scale(' + zoom + ')' });
    $('.placeDitailsContainer').css({ 'top': 80 + 455 * zoom + 'px' });
}
