...
function createDocument(result, response) {
    ...
    fileSystem.writeFile(path.join(__dirname, "../public/readme.kml"), data, function (err) {
        if (err) {
    ...
    //fileSystem.open(path.join(__dirname, "../public/readme.kml"), "w", 0644, function (err, file_handle) {
    //    if (!err) {
    ...
    //        fileSystem.writeFile(file_handle, data, null, 'utf8', function (err, written) {
    //            if (!err) {
...
function sendKmlFile(response) {
    var filePath = path.join(__dirname, "../public/readme.kml");
